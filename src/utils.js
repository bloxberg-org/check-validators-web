...
export function getLastBlock(address) {
  ...
  return fetch('https://blockexplorer.bloxberg.org/api/api?module=account&action=getminedblocks&address=' + address + '&page=1&offset=1')
    .then(res => {
      return res.json()
    })
...
export function getBlocks(addr, page = 1, offset = 1000) {
  ...
  return fetch(`https://blockexplorer.bloxberg.org/api/api?module=account&action=getminedblocks&address=${addr}&page=${page}&offset=${offset}`).then(res => res.json()).then(json => json.result);
}